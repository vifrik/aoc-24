...
class Grid:
    ...
    def _parse(self, input: str) -> None:
        ...
        for line_idx, line in enumerate(lines):
            ...
            for val_idx, val in enumerate(line):
                datum = Datum(val_idx, line_idx, val)
                inner.append(datum)
                point = datum.to_point()
                self._point_map[point] = line_idx, val_idx
                self._unique_vals.add(val)
                if val not in self._val_positions:
                    self._val_positions[val] = list()
                self._val_positions[val].append(point)
            outer.append(inner)
        self._grid = outer
    ...
    def set(self, point: Point, value: str) -> None:
        ...
        old_value = self._grid[point.y][point.x].v
        self._val_positions[old_value].remove(point)
        self._val_positions[value].append(point)
        y, x = self._point_map[point]
